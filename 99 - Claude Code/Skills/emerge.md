...
**Entrée** : l'utilisateur lance `/emerge` ou système propose après `/harvestdeep`

**Sortie** : Max 5 clusters proposés avec score/confiance/notes sources, attente validation de l'utilisateur avant création

**Temps** : ~1-2h complet (dépend du vault — Pass 1B/1C sont les phases lourdes)

**Étapes clés** :
1. **Lire références** (5-10 min) — INDEX, profil utilisateur, command-tracker
...
6. **Présentation** (5 min) — montrer top 5 à l'utilisateur
7. **Création + linking** (15-20 min) — après validation de l'utilisateur

## Déclenchement

- **Manuel** : l'utilisateur dit "/emerge", "fais un emerge", "qu'est-ce qui émerge du vault"
...
```
1. `[PROJECTS_FOLDER]/INDEX.md` → projets actifs (exclure de l'analyse)
2. `[ME_FOLDER]/[NOM].md` → profil, context (pour interpréter signaux)
...
- Une note marquante par domaine ([ME_FOLDER]/, [HOBBIES_FOLDER]/, [KNOWLEDGE_FOLDER]/, [PROJECTS_FOLDER]/Project ideas/, Sessions/)
...
**Si 0 candidats** : demander à l'utilisateur (continuer Phase 1C ou arrêter ?). **Si > 15** : trier par domaines, top 15.
...
**Processus** :
1. Chercher l'idée telle qu'elle serait formulée : lire `[PROJECTS_FOLDER]/INDEX.md` (projet actif?), `[PROJECTS_FOLDER]/Project ideas/` (idea note existante?), `[KNOWLEDGE_FOLDER]/` (concept déjà synthétisé?)
...
| Score | Niveau |
|-------|--------|
| 6+ | **Haut** — fort signal, prêt à créer ou valider avec l'utilisateur |
| 4-5 | **Moyen** — signal clair mais incomplet, à approfondir avec l'utilisateur |
...
| Signal | Output suggéré | Cas l'utilisateur |
|--------|----------------|-----------|
| **Cluster d'idées techniques / product** — pointe vers un outil, système, app ou SaaS à construire | **Projet** → note dans `[PROJECTS_FOLDER]/Project ideas/` | Outils dev, automation, infra |
| **Cluster de réflexions, observations, tensions** — observations sur la vie, le travail, la cognition ou concepts sans déliverable concret | **Essay** → texte narratif dans `[PROJECTS_FOLDER]/Project ideas/` | réflexions personnelles, systèmes, processus, réflexions |
| **Cluster ambigu** — pas encore assez clair si c'est un projet ou une réflexion | **Prodemander** : "Je vois deux directions, laquelle tu veux explorer?" |
...
## Étape 6 — Présentation à l'utilisateur
...
Attendre validation de l'utilisateur avant création.
...
**Projet** — note dans `[PROJECTS_FOLDER]/Project ideas/[slug-titre].md` :
...
[Thèse, argument — à remplir par l'utilisateur]
...
Utiliser **Write tool** (pas Obsidian API). Chemin : `[PROJECTS_FOLDER]/Project ideas/[slug-titre].md`
...
2. Proposer l'utilisateur emplacements précis par source (quelle section, quelle phrase)
...
| **0 candidats après Phase 1B** | Demander l'utilisateur : "Continuer Phase 1C (détail) ou arrêter?" |
...
| **Timeout > 1h** | Limiter Phase 1 à 30j (au lieu de complet), signaler l'utilisateur |
| **Borderline (Étape 3)** | Demander l'utilisateur : "Rework existante ou émergeant?" |
...
- **Jamais créer sans validation explicite** de l'utilisateur
...
- **Jamais proposer quelque chose qui est déjà un projet actif** dans `[PROJECTS_FOLDER]/INDEX.md`
...
- [ ] Validation l'utilisateur avant création (Étape 7)
