---
name: vault-link
description: Analyser le vault et créer des liens [[]] entre les notes. Déclenche ce skill quand l'utilisateur dit "/link", "fais le linking", "relie mes notes" ou "vault link".
---
# Skill : Vault Link

Ce skill analyse le vault et propose des liens Obsidian `[[]]` entre les notes qui parlent des mêmes sujets. Il ne modifie jamais une note sans validation explicite de l'utilisateur.

Philosophie d'ancrage : le rôle est de rendre le graphe traversable, pas de générer de la compréhension. Claude connecte. l'utilisateur pense. Un graphe bien traversable permet à d'autres commandes comme `/emerge`, `/trace` ou `/connect` de fonctionner correctement — c'est l'enjeu réel du linking.
...
- l'utilisateur dit "/link", "fais le linking", "relie mes notes"
- l'utilisateur précise un dossier : "/link mes notes Warhammer"
- Sans précision → analyser tout le vault

## Heuristiques apprises — refus de l'utilisateur
...
Deux projets distincts peuvent partager un sujet commun (ex : un hobby et un projet logiciel liés à la même thématique), mais ce sont deux projets indépendants dans le vault. Ne pas créer de liens croisés entre eux — le sujet commun ne suffit pas, il faut que les notes se complètent fonctionnellement.

### Ne pas lier l'organisation d'un projet avec les notes personnelles d'un autre domaine
La note `Event warhammer.md` (organisation MonAsso) ne doit pas pointer vers `suivi-armees.md` (hobby perso). La gestion associative et la collection personnelle sont deux contextes séparés, même si le sujet (Warhammer) est le même.
...
### Si l'utilisateur refuse l'ensemble du résultat
...
Si l'utilisateur précise un dossier ou une note → se limiter à ce périmètre. Sans précision → scanner tous les dossiers sauf `[NOTES_FOLDER]/` (trop de volume, liens éphémères), en incluant obligatoirement `99 - Claude Code/Sessions/`.

Lister les dossiers qui seront analysés et attendre confirmation de l'utilisateur.
...
**Si des liens brisés existent :** présenter à l'utilisateur avec raison supposée ("note à créer" vs "lien mal formé"). Attendre instruction avant d'agir.
...
3. Notes dans les dossiers actifs (`[KNOWLEDGE_FOLDER]/`, `[PROJECTS_FOLDER]/`, `99 - Claude Code/`) — ces dossiers contiennent les notes les plus récemment modifiées par défaut
...
**Cas "aucun lien pertinent" :** si aucun lien candidat n'atteint score 5, indiquer à l'utilisateur "aucun lien proposé pour ce périmètre — notes trop isolées ou déjà bien connectées" et s'arrêter (ne pas continuer à l'Étape 5).
...
|-------------|----------------|------------|--------|-------|
| vault-second-brain.md | [[profil]] | profil-utilisateur.md | Contexte personnel directement lié | 20 |
| NMM.md | [[Balthasar-Gold]] | Balthasar-Gold.md | Peinture mentionnée dans la technique | 16 |
```

**Attendre la validation de l'utilisateur avant d'ajouter quoi que ce soit.** l'utilisateur peut rejeter des liens individuellement ou l'ensemble des suggestions.

## Étape 5 — Ajout des liens et nettoyage

Pour chaque lien validé par l'utilisateur, utiliser Edit pour ajouter `[[nom-note]]` à l'endroit pertinent dans la note source — en fin de note dans une section `## Voir aussi` si aucun endroit naturel n'existe.
...
**Après ajout de tous les liens :** mettre à jour le tracker (`99 - Claude Code/command-tracker.md`) si le fichier existe — ligne `/link` → remplacer la date par la date du jour au format `YYYY-MM-DD`. Ne pas bloquer si le tracker n'existe pas.

## Étape 6 — Capitalisation des refus (optionnel)

À chaque fois que l'utilisateur refuse un lien (ou l'ensemble des suggestions) avec une explication, proposer de capitaliser : ajouter l'heuristique dans la section "Heuristiques apprises" avec le pattern général (pas l'exemple spécifique) pour que la règle s'applique aux prochaines sessions.

**Si l'utilisateur refuse l'ensemble du résultat :** noter le contexte (type de notes, périmètre, thème, raison du refus) — ne pas relancer une session similaire sans signal explicite.
