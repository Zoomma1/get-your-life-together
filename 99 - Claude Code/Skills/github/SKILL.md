...
Unified interface for GitHub operations. All commands use `gh` CLI or `gh api` — no MCP dependencies.

**Prerequisite**: `gh` authenticated (`gh auth status` must return an active account).
...
1. Calculate the date from 2 months ago (from today) in `YYYY-MM-DD` format
2. Run:
   ```bash
...
3. Display:
   ...
   ```
4. {USER_NAME} answers with the number, partial name, or a name not in the list
...
**Repo retained in session** — don't ask again as long as {USER_NAME} doesn't explicitly change repos.
...
1. Request confirmation: "Create issue '[title]' on [owner/repo]?"
...
If branch not specified → use `default_branch` retained in session.

1. List the latest runs on the branch:
   ```bash
...
If path not specified → repo root (`/`).
...
Display content in a code block with the right language by file extension.

**If the file is too large** (> 1 MB) → signal and offer to read a section via `--jq '.download_url'` then `gh api [url]`.

---

## Absolute rules

- **Confirmation mandatory** before any write action (`create issue`, `comment`)
- **Repo and default_branch retained in session** — Action 0 selection once per session unless {USER_NAME} explicitly changes repos
- **Path encoded** for `gh api`: spaces → `%20` (e.g., `99%20-%20Claude%20Code/Skills`)
- **Issue vs PR detection**: try `gh pr view` first, fallback `gh issue view` if error
