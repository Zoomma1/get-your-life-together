...
- Si **non** → inviter Claude à lancer `/digest` avec instruction "continue sans attendre la réponse si elle prend > 5s, le digest s'ajoute à la note après coup"
  - Ce n'est pas un étape bloquante — la planification continue en parallèle
  - Aucun timeout du côté skill, le digest s'exécute indépendamment
...
`pool_max = énergie × 2` (total toutes sections confondues)

Défaut : si énergie non renseignée ou non numérique → utiliser `énergie = 3`, donc `pool_max = 6`.

Le pool_max est le plafond absolu du nombre de tâches proposées dans le plan principal (toutes sections confondues, excl. Raffinement).

**Calculer les budgets par contexte** :

```
budget_perso  = personal_hours
...
2. Pool global `pool_max` atteint (toutes sections confondues), OU
3. Plus de candidats disponibles
...
- **Couverture obligatoire de tous les kanbans** : chaque kanban actif (projets actifs + Claude Code Kanban + Hobby Kanban) doit contribuer au moins 1 suggestion si des tickets WIP ou Ready sont disponibles et non bloqués (sauf si pool_max déjà atteint)
...
- **La daily note n'est écrite qu'après validation** : écrire uniquement les tâches que Victor a retenues (budget par contexte + pool_max, premier plafond l'emporte). La section Raffinement est hors quota.
...
La section `🗂️ Raffinement` **ne compte pas** dans les limites `pool_max` et budgets contexte. Toujours affichée si ≥ 1 ticket.
...
1. Compter les tâches du plan principal (hors Raffinement) → si > `pool_max` : supprimer les dernières de la liste jusqu'à conformité
2. Pour chaque contexte (Travail / Perso / Hobby) : sommer les durées estimées → si **strictement supérieur** au budget du contexte (budget résiduel < 0) : supprimer la dernière tâche ajoutée et réessayer la suivante. Budget = 0 après addition = exact fit, conserver.
3. Si une tâche n'a pas de durée estimée → lui attribuer 45min avant de sommer
4. **Jamais présenter un plan non conforme** — tronquer d'abord, présenter ensuite
...
- **Pool énergie × 2** — pool total toutes sections = `énergie × 2` candidats (défaut 6 si énergie non fournie). Budget par contexte = heures frontmatter. Le premier plafond atteint (pool_max ou budget épuisé) stoppe la sélection. La section Raffinement est hors quota.
