---
name: process
description: Give a URL → Claude fetches the content and creates a structured note in 03 - Knowledge/ immediately. Use when the user says "/process [url]" or "process this link", "capitalize this article/this video". Bypasses harvest for immediate processing.
---
...
- If {USER_NAME} cancels at any time (Ctrl+C) → stop immediately and signal cancellation
...
3. If valid response (clean markdown, > 100 chars) → use defuddle content. Retrieve defuddle frontmatter (`title`, `author`, `site`, `word_count`) to pre-populate note metadata.
...
1. **n8n fallback** — verify that N100 is reachable:
   ```bash
   ...
   curl -s -X POST http://100.75.159.73:5678/webhook/fallback-link \
     ...
     -d "{\"url\": \"[URL]\", \"type\": \"[type]\"}"
   ```
   Analyze response: if `{ "title": "...", "content": "..." }` with content >= 100 chars → continue.
...
steps in order, practical advice. Be factual and concise.
```

---

## Step 2 — Identify the type and domain

From the obtained content, determine:
- **Type**: `article`, `video`, `forum`, `doc`, `podcast`
- **Domain**: see table below

**Domain → subfolder mapping:**
...
If `--target` provided → use this path directly. Create the folder if it doesn't exist (`mkdir -p`). Accept any valid path under `03 - Knowledge/` or `02 - Hobbies/`.

---

## Step 3 — Check for duplicates

List existing files in the target subfolder.
...
- `## Use cases with my workflow`: concrete links (FSTG, vault, projects) — omit if no obvious link
- `## See also`: search vault for related notes. At least 1 link if relevant exists, otherwise omit this section.
